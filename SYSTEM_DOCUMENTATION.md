...
7️⃣ Dashboard-Linked Feature Components (In-Depth)

Block Name: Clubs Module (`/clubs` → `/club-profile`)
- Components involved:
  - `ClubSearchBar`: query input + clear button for club filtering.
  - `ClubCard`: logo + club name tile; press navigates to club profile.
  - `ClubSearchEmptyState`: empty-state guidance for no search matches.
  - `ClubLogo`: image rendering with fallback initials.
  - `EditableTextSection`: editable/read-only About Us, What to Expect, Achievements.
  - `ClubGallery`: gallery upload/delete/fullscreen preview modal.
  - `ClubFilesSection`: PDF/image file uploads and file list actions.
- Functional behavior:
  - Clubs list loads from `clubs` table with schema fallback (`logo_url` optional).
  - Latest logo can also be resolved from `club_files` records tagged as logo.
  - Club profile subscribes to realtime postgres changes on `clubs`, `club_sections`, and `club_files`.
  - Manager-only editing is controlled by `canManageClub(clubId)`.
- Edit capabilities (manager only):
  - Club name edit.
  - Club logo upload/change/remove with validation (JPEG/PNG, max 2MB).
  - Section text edit for About/What to Expect/Achievements.
  - Gallery and file management.
- Security + RLS dependencies:
  - `clubs`, `club_sections`, `club_files`, and storage bucket policies must permit role-appropriate access.
  - Non-managers are read-only and see “Not authorized to edit this club.”

Block Name: Events + Attendance Module (`/student/events` → `/student/events/[id]`)
- Components involved:
  - Event list card rendering with search + sort controls.
  - `EventRegistrationModal`: registration form and submission.
  - Attendance submit modal in Event Details.
- Functional behavior:
  - Student event list shows active upcoming events from `getEventsForStudent()`.
  - Event details loads event + registration + local registration cache + attendance-marked state in parallel.
  - If user is not registered, registration modal auto-prompts once.
  - Attendance button is enabled only when:
    1) user registered,
    2) QR enabled for event,
    3) current time within event window,
    4) attendance not already marked.
  - Submit path calls `submitAttendance(eventId)` and rechecks `hasMarkedAttendance`.
- Validation and states:
  - Registration required before attendance submission.
  - Attendance modal shows registered USN read-only.
  - Handles duplicate submission (`already`) and forbidden (`42501`) path-specific messaging.

Block Name: Attendance History Module (`/attendance-history` student mode)
- Components involved:
  - `AttendanceList` (student-focused event attendance list).
- Functional behavior:
  - Fetches all events and user’s attendance scans.
  - Merges into event list with attended/not-attended status per event.
  - No edit actions; view-only historical status.

Block Name: Notifications Module (`/notifications`)
- Components involved:
  - Flat list cards + optional add/delete controls based on role.
- Functional behavior:
  - Student sees today’s visible notifications (role-filtered service behavior).
  - Realtime subscription to `notifications` table refreshes feed.
  - Student role has no compose/delete actions.

Block Name: Account & Settings Module (Dropdown)
- Components linked:
  - `/edit-profile`: profile name/email fetch + update.
  - `/change-password`: old/new/confirm validation + reauthentication + password update.
  - `/calendar`: `CalendarGrid` day/event visualization.
- Critical behaviors:
  - Change password can force logout/login refresh path.
  - Calendar view is read-only schedule representation.
  - Theme toggle changes client theme context only (no backend writes).
...
Block Name: Clubs Management Module (`/clubs` → `/club-profile`)
- Components involved:
  - `ClubSearchBar`, `ClubCard`, `ClubSearchEmptyState`.
  - In profile: `ClubLogo`, `EditableTextSection`, `ClubGallery`, `ClubFilesSection`.
- Functional behavior:
  - Faculty/admin can access clubs list and open profile pages.
  - Manage permissions are resolved by `canManageClub(clubId)`; admin typically passes by role.
  - Editable areas include club name/logo/section text and media/file libraries.
- Club logo details:
  - Upload path writes to Supabase storage bucket `club-logos`.
  - Metadata entry written in `club_files` with logo tags.
  - Existing logo fallback logic supports `clubs.logo_url` and latest logo file.
- Validation/constraints:
  - Logo type restricted to JPEG/PNG; size capped to 2MB.
  - File/gallery validations enforced in component modules (format + size + item limits).

Block Name: Event Management Module (`/event-management`)
- Components involved:
  - `EventCreationModal` for create flow.
  - Event cards with Manage Attendance + Delete actions.
- Functional behavior:
  - Uses `getMyManagedClubIds()` and `listEventsForClubIds()` to scope visible events.
  - Create event validates title/date/time/location and end-time-after-start.
  - Delete event is soft-delete via service (`status='deleted'`).
- Attendance control linkage:
  - Manage Attendance opens `/president/events/[id]` detail view (shared manager flow).

Block Name: Attendance Controls per Event (`/president/events/[id]` shared for managers)
- Components involved:
  - Detail rows + two action buttons:
    - Generate Attendance (enable QR)
    - Disable Attendance (disable QR)
- Functional behavior:
  - Validates manager permission via `canManageClub(event.club_id)`.
  - Enabling creates secure QR token and marks `qr_enabled=true`.
  - Disabling sets `qr_enabled=false` while preserving event details.
  - Students’ attendance availability depends directly on this toggle.

Block Name: Attendance History Module (`/attendance-history` faculty/admin mode)
- Components involved:
  - `EventList` (select event),
  - `AttendanceStudentList` (students scanned),
  - search inputs for events and students.
- Functional behavior:
  - Event-first workflow: select event then inspect attendees.
  - Attendance data sourced via RPC `get_attendance_history_for_club` and filtered by selected event.
  - Search supports title/date/club for events and USN/name/email for students.

Block Name: Notifications Management Module (`/notifications` + `/compose-notification`)
- Inbox components:
  - notification cards with delete icon.
- Composer components:
  - club chips selector, title/body inputs, send button.
- Functional behavior:
  - `getNotificationClubOptions` scopes clubs by role assignment.
  - Per-club daily cap enforced by `getNotificationCountForClubToday` (<2).
  - Send uses `sendNotification` and permission checks.
  - Delete uses `deleteNotification` with explicit role + RLS handling.

Block Name: Account/Utility Components (Dropdown)
- `/edit-profile`: profile read/update flow for name/email.
- `/change-password`: secure password change workflow with visibility toggles and validations.
- `/calendar`: role-aware event calendar rendered through `CalendarGrid`.
- Theme toggle + logout remain global dashboard utilities.
...
Block Name: Clubs Management Module (`/clubs` → `/club-profile`)
- Same component stack as faculty/admin:
  - `ClubSearchBar`, `ClubCard`, `ClubSearchEmptyState`.
  - `ClubLogo`, `EditableTextSection`, `ClubGallery`, `ClubFilesSection`.
- President-specific operational note:
  - President typically has a single assigned club via `president_assignments`; manage rights are evaluated through `canManageClub`.
- Editable capabilities:
  - club name, logo management, section text, gallery, and files (when manager permission true).

Block Name: Event Management Module (`/event-management`)
- Components involved:
  - `EventCreationModal` (+ Add Event), event cards, manage/delete buttons.
- Functional behavior:
  - President loads manageable clubs and active events.
  - Event create uses validated form data and assigned club constraints.
  - Event delete is managed/authorized soft delete.

Block Name: Attendance Toggle/Control Module (`/president/events/[id]`)
- Components involved:
  - Event detail metadata section.
  - Attendance control buttons:
    - “Generate Attendance” (enable)
    - “Disable Attendance” (disable)
- Functional behavior:
  - Permission check via `canManageClub`.
  - Enable flow calls `enableEventAttendance` (QR token generation + qr_enabled true).
  - Disable flow calls `disableEventAttendance` (qr_enabled false).
  - Student attendance submission eligibility is downstream-dependent on this state.

Block Name: Attendance History Module (`/attendance-history` president mode)
- Components involved:
  - `EventList`, `AttendanceStudentList`, search inputs.
- Functional behavior:
  - Event selection reveals attendee scans.
  - Data returned by RPC and client-filtered per event.
  - Useful for validating event participation and scan completeness.

Block Name: Notifications Module (`/notifications`, `/compose-notification`)
- Components involved:
  - Inbox list + delete action.
  - Composer with club selection chips, title/body inputs, daily-limit message.
- Functional behavior:
  - President can send/delete within assigned club scope.
  - Daily cap per club enforced at 2 notifications.
  - Realtime inbox refresh via notifications channel.

Block Name: Governance Transfer Module (`/change-president`)
- Components involved:
  - Email input for successor.
  - Confirm input requiring exact “YES”.
  - Transfer button with loading state.
- Functional behavior:
  - Resolves current president’s assigned `club_id`.
  - Calls RPC `transfer_president_by_email` with strict confirmation payload.
  - On success: alert + force signOut + redirect login.
- Validation/security:
  - Email format validation.
  - Confirmation text gate.
  - Session validity and assignment existence checks before RPC.

Block Name: Account/Utility Components (Dropdown)
- `/edit-profile`, `/change-password`, `/calendar`, theme toggle, logout.
- These utilities mirror other dashboards but remain president-accessible while logged in.
